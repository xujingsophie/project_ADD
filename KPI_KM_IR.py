...
ikl = IsolationForest(bootstrap=False,contamination=0.05)
...
# 预测
iris_train = ikl.predict(data2)
# 预测分数 分越低越可能是异常
iris_score = ikl.decision_function(data2)
...
y_result = []
for i in X_test:
    y_result.append(float(i))
y_result.insert(0, '孤立森林检测结果')
X_save = np.column_stack((data, y_result))

lines_2 = pd.DataFrame(X_save)
lines_2.to_csv("E:/异常检测/小区分场景聚类/bj_data/bjkpi33删除空字段_IRresult_KM41009.csv", header=0, index=0)  # 不保留行索引和列名
