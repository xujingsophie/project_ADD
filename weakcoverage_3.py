...
def demoTest1(provience, num):
    ...
    for line in lines[1:len(lines) - 1]:
        ...
        if (p[0] != provience):
            ...
            # ['FJ', '南平', '邵武余山ZLH1-2', '117.10498', '27.15513', '25196-131', '166266', '0.2573', 'outdoor', '0']
            try:
                if (int(p[sam_point]) >= 1000 and float(p[cov_rat]) <= 0.8) and p[inout_door] == 'outdoor' and p[core_city] == '1':
                    # print(p)
                    for i in range(len(p)):
                ...
                continue
                # print(j)
            if (int(p[sam_point]) >= 1000 and float(p[cov_rat]) <= 0.9) and p[inout_door] == 'indoor':
                for i in range(len(p)):
                    ws2.cell(row=z + 1, column=i + 1).value = p[i]
                z = z + 1
        else:
